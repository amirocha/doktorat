...
rms = 7.530E-03 # rms taken from CLASS
rms_3 = 3*rms
rms_15 = 30*rms

# read the spectrum
spec_df = pd.read_table('co21_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > 5.) & (spec_df[0] < 15.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 15) & (spec_df[0] < 30)]
...
x3_ran_df = spec_df[(spec_df[0] > 5) & (spec_df[0] < 15)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 15) & (spec_df[0] < 29.5)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('co21_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
...
major_ticks_x = np.arange(-20, 40, 5)                                              
minor_ticks_x = np.arange(-20, 20, 1) 
