...
psname = 'serpens_h13cn10_3s_main.eps'
...
rms = 2.818E-02 # rms taken from CLASS
rms_3 = 3*rms

# read the spectrum
spec_df = pd.read_table('serpens_h13cn10_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > 5.0) & (spec_df[0] < 10.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 20) & (spec_df[0] < 24.)]
...
x3_ran_df = spec_df[(spec_df[0] > 5.0) & (spec_df[0] < 10.0)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 20.0) & (spec_df[0] < 24.0)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('serpens_h13cn10_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
