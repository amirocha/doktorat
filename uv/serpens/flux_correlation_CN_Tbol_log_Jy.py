...
def plot_correlation(x, y, a, b, pearson):
	...
	major_ticks_x = np.arange(-1, 3, 0.5)
	...
	plt.savefig('CN_Tbol_correlation_log_v2', format='eps')
	plt.close()
...
def main():
	...
	JytoK = estimate_conversion_factor(freq['CN'])
	Lbol_source = calculate_Lbol_for_molecule(fluxes, 'CN', JytoK)
	...
	Tbol, CN_list = sort_points(Tbol, CN_list)
	Tbol = make_log_list(Tbol)
	CN_list = make_log_list(CN_list)

	a, b, stderr = fit_linear_regression(Tbol, CN_list)
	pearson = calculate_pearson(Tbol, CN_list)
	plot_correlation(Tbol, CN_list, a, b, pearson)
