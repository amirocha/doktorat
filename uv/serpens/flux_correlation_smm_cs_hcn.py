...
def create_flux_file(data):
	...
	for i in range(1,len(data)):
		...
		if line[1] == 'cs32' and previous_line[1] == 'hcn10':
...
def plot_correlation(x, y, a, b):
	...
	ax.set_ylabel("I(CS J=3-2) [K km/s]")
	...
	sources_list=['SMM5','SMM8','SMM10','SMM1','SMM2','SMM9','','','SMM3','SMM4']
	for i in range(len(sources_list)):
		ax.text(x[i]-0.5, y[i]+0.2, sources_list[i])
	...
	plt.savefig('CS_HCN_flux_correlation_smm', format='eps')
	plt.close()
...
def main():
	...
	write_data('CS_HCN_corr_smm', fluxes, a, b, pearson, stderr)
