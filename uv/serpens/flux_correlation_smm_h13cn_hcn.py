...
def create_flux_file(data):
	...
	for i in range(4,len(data)):
		line = data[i].split()
		previous_line = data[i-3].split()
		if line[1] == 'h13cn10' and previous_line[1] == 'hcn10':
...
def plot_correlation(x, y, a, b):
	...
	ax.set_ylabel(r"I(H$^{13}$CN J=1-0) [K km/s]")
	...
	sources_list=['SMM5','SMM8','SMM10','SMM1','SMM2','SMM9','SMM12','SMM6','SMM3','SMM4']
	for i in range(len(sources_list)):
		ax.text(x[i], y[i], sources_list[i])
	...
	plt.savefig('H13CN_HCN_flux_correlation_smm', format='eps')
	plt.close()
...
def main():
	...
	write_data('H13CN_HCN_corr_smm', fluxes, a, b, pearson, stderr)
