...
rms = 7.677E-02 # rms taken from CLASS
rms_3 = 3*rms
rms_2 = 2*rms

# read the spectrum
spec_df = pd.read_table('serpens_hco+43_smm4.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > 0.0) & (spec_df[0] < 8.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 8) & (spec_df[0] < 15.)]
...
x3_ran_df = spec_df[(spec_df[0] > 0.0) & (spec_df[0] < 8.0)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 8.0) & (spec_df[0] < 15.1)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('serpens_hco+43_smm4.txt', usecols=(0, 1), unpack=True, skiprows=1)
