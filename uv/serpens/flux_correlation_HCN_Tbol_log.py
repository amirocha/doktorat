...
def plot_correlation(x, y, a, b, pearson):
	...
	ax.set_ylabel("$\log(\mathrm{I(HCN \; 1-0)})$ [K km/s]")
	ax.set_xlabel(r"$\log(T_{\mathrm{bol}}$) [K]")

	major_ticks_x = np.arange(0, 3, 0.5)
	major_ticks_y = np.arange(0, 2, 0.2)
	...
	plt.savefig('HCN_Tbol_correlation_log', format='eps')
	plt.close()
...
def main():
	...
	Tbol, HCN_list = sort_points(Tbol, HCN_list)
	Tbol = make_log_list(Tbol)
	HCN_list = make_log_list(HCN_list)

	a, b, stderr = fit_linear_regression(Tbol, HCN_list)
	pearson = calculate_pearson(Tbol, HCN_list)
	plot_correlation(Tbol, HCN_list, a, b, pearson)
