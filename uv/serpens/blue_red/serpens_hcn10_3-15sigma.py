...
psname = 'serpens_hcn10_smm1_3s_15s_comp1.eps'
...
rms = 4.893E-02 # rms taken from CLASS
rms_3 = 3*rms
rms_15 = 45*rms

# read the spectrum
spec_df = pd.read_table('./serpens_hcn10_smm1.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > 10) & (spec_df[0] < 11.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 15) & (spec_df[0] < 17)]
...
x3_ran_df = spec_df[(spec_df[0] > 11) & (spec_df[0] < 13)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 13) & (spec_df[0] < 15)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('./serpens_hcn10_smm1.txt', usecols=(0, 1), unpack=True, skiprows=1)
...
major_ticks_x = np.arange(-95, 95, 1)                                              
minor_ticks_x = np.arange(-95, 95, 1) 

major_ticks_y = np.arange(0.0, 5, 0.1)                                              
minor_ticks_y = np.arange(0.0, 5, 0.1) 
