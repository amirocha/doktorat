...
rms = 1.231E-02 # rms taken from CLASS
rms_3 = 3*rms

# read the spectrum
spec_df = pd.read_table('serpens_c34s32_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > 0.0) & (spec_df[0] < 5.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 10) & (spec_df[0] < 17.)]
...
# SERPENS CS32: -0.0 - 5.0  and 10. - 15.
# SERPENS C34S32: -0.0 - 5.0  and 10. - 17.
...
x3_ran_df = spec_df[(spec_df[0] > 0.0) & (spec_df[0] < 5.0)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 13.0) & (spec_df[0] < 17.1)]

#change ranges!!
# NGC1333 HCN10: -30.0 - -20.0  and -5. - -1.
# Serpens C34S32: 0 - 5 and 13 - 17
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('serpens_c34s32_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
...
major_ticks_x = np.arange(-50, 50, 5)                                              
minor_ticks_x = np.arange(-50, 50, 1) 
