...
# read the spectrum
spec_df = pd.read_table('serpens_pos2_hcn10.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > 0) & (spec_df[0] < 3.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 12) & (spec_df[0] < 20.)]
...
max1 = final1_df[3].idxmax(axis=1, skipna=True)
max2 = final2_df[3].idxmax(axis=1, skipna=True)


# print the x value of minimum (in km/s)
print ('X1 (3s) =', final1_df[1].ix[max1].round(1))
print ('X2 (3s) =', final2_df[1].ix[max2].round(1))
...
x3_ran_df = spec_df[(spec_df[0] > -7.0) & (spec_df[0] < -5.0)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 15.0) & (spec_df[0] < 20.1)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('serpens_pos2_hcn10.txt', usecols=(0, 1), unpack=True, skiprows=1)
...
plt.axvline(x=final1_df[1].ix[max1].round(1), color='red', linestyle='--')
plt.axvline(x=final2_df[1].ix[max2].round(1), color='red', linestyle='--')
