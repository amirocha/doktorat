...
rms = 9.146E-02 # rms taken from CLASS
rms_3 = 3*rms

# read the spectrum
spec_df = pd.read_table('serpens_c18o65_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > -5.0) & (spec_df[0] < 5.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 10) & (spec_df[0] < 20.)]

#change ranges!!
# SERPENS HCN10: -5.0 - -0.0  and 10. - 20.
# Smm1 co65: -5.0 - 5.0  and 10. - 20.
...
x3_ran_df = spec_df[(spec_df[0] > 6) & (spec_df[0] < 7.0)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 10.0) & (spec_df[0] < 12)]

#change ranges!!
# NGC1333 HCN10: -30.0 - -20.0  and -5. - -1.
#SMM1 C18O: 6-7 and 10-12
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('serpens_c18o65_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
...
major_ticks_y = np.arange(-10.0, 1.2, 0.2)                                              
minor_ticks_y = np.arange(-10.0, 1.2, 0.1) 
