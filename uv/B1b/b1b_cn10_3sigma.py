...
rms = 8.477E-02  # rms taken from CLASS
rms_3 = 3*rms
rms_15 = 15*rms

# read the spectrum
spec_df = pd.read_table('./b1b_cn10_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > -5) & (spec_df[0] < -4)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > -3) & (spec_df[0] < -2)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('./b1b_cn10_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
