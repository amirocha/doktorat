...
rms = 1.853E-02 # rms taken from CLASS
rms_3 = 3*rms

# read the spectrum
spec_df = pd.read_table('b1b_hcn10_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > -21.0) & (spec_df[0] < -20.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > -7) & (spec_df[0] < -5.)]

#change ranges!!
# SERPENS HCN10: -21.0 - -20.0  and -7. - -5.
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('b1b_hcn10_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
