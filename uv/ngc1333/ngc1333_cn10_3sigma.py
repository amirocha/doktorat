...
rms = 6.874E-02 # rms taken from CLASS
rms_3 = 3*rms
rms_15 = 15*rms

# read the spectrum
spec_df = pd.read_table('./ngc1333_cn10_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > -93.) & (spec_df[0] < -88.5)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > -87.) & (spec_df[0] < -83)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('./ngc1333_cn10_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
...
major_ticks_x = np.arange(-180, 15, 1)                                              
minor_ticks_x = np.arange(-185, 15, 1) 
