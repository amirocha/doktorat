...
rms = 1.913E-02 # rms taken from CLASS
rms_3 = 3*rms
rms_15 = 15*rms

# read the spectrum
spec_df = pd.read_table('./ngc1333_h13cn10_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > -15.) & (spec_df[0] < -10)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > 0) & (spec_df[0] < 5)]
...
y1_i_rms_3 = (x1_ran_df[1]-rms).abs()
y2_i_rms_3 = (x2_ran_df[1]-rms).abs()
...
x3_ran_df = spec_df[(spec_df[0] > -15.5) & (spec_df[0] < -10)]  #change ranges!!
x4_ran_df = spec_df[(spec_df[0] > 0) & (spec_df[0] < 5)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('./ngc1333_h13cn10_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
...
major_ticks_x = np.arange(-80, 85, 10)                                              
minor_ticks_x = np.arange(-85, 85, 10) 
...
plt.axvline(x=final2_df[1].ix[min2].round(1), color='blue', linestyle='--')

# plot the vertical lines for x = min3 and x = min4
plt.axvline(x=final3_df[1].ix[min3].round(1), color='red', linestyle='--')
