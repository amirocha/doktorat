...
def make_picture(X, Y, Z):
	...
	fig, ax = plt.subplots()
	CS = ax.contour(X, Y, Z, levels = [1000, 10000, 100000, 1000000, 10000000])
	ax.clabel(CS, inline=1, fontsize=10)
	observations = ax.contourf(X, Y, Z, levels = [1,10])
	ax.set_title(r'Chemical model parameters - C/C2 ratio')
	...
	plt.savefig('contour_plot_C_C2.eps')
	plt.close()
	...
	for dens in N:
		...
		for av in AV_tot:
			if av in AV:
				...
				time_position = 124  
				C2_abudance = data[29].split()[time_position]  
				C2_abudance = remove_D_from_elem(C2_abudance)
				C_abudance = data[5].split()[time_position] 
				C_abudance = remove_D_from_elem(C_abudance)
				abudance_ratio = float(C_abudance)/float(C2_abudance)
				Z_rows.append(abudance_ratio)
			elif av not in AV:
				av_new = AV_new[i]
				data_new = read_data('./plot'+dens+'-'+av_new+'.dat')
				time_position = 124  
				C2_abudance_new = data_new[29].split()[time_position]
				C2_abudance_new = remove_D_from_elem(C2_abudance_new)
				C_abudance_new = data_new[5].split()[time_position] 
				C_abudance_new = remove_D_from_elem(C_abudance_new)
				abudance_ratio_new = float(C_abudance_new)/float(C2_abudance_new)
