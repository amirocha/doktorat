...
def make_picture(X, Y, Z):
	...
	fig, ax = plt.subplots()
	CS = ax.contour(X, Y, Z, levels = [0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000])
	ax.clabel(CS, inline=1, fontsize=10)
	observations = ax.contourf(X, Y, Z, levels = [1,10])
	ax.set_title(r'Chemical model parameters - C/C+ ratio')
	...
	plt.savefig('contour_plot_C_C+.eps')
	plt.close()
	...
	for dens in N:
		...
		for av in AV_tot:
			if av in AV:
				...
				time_position = 124  
				Cplus_abudance = data[194].split()[time_position]  
				Cplus_abudance = remove_D_from_elem(Cplus_abudance)
				C_abudance = data[5].split()[time_position] 
				C_abudance = remove_D_from_elem(C_abudance)
				abudance_ratio = float(C_abudance)/float(Cplus_abudance)
				Z_rows.append(abudance_ratio)
			elif av not in AV:
				av_new = AV_new[i]
				data_new = read_data('./plot'+dens+'-'+av_new+'.dat')
				time_position = 124  
				Cplus_abudance_new = data_new[194].split()[time_position]
				Cplus_abudance_new = remove_D_from_elem(Cplus_abudance_new)
				C_abudance_new = data_new[5].split()[time_position] 
				C_abudance_new = remove_D_from_elem(C_abudance_new)
				abudance_ratio_new = float(C_abudance_new)/float(Cplus_abudance_new)
