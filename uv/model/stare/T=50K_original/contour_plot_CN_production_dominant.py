...
def make_picture(grids, action, mol, reaction, method):
	...
	if action == 'production' and mol == 'HCN':
		ax.text(-3.7, 5.5, r'N+CH$_2$$\rightarrow$H+HCN', fontsize=12, color='white')
		ax.text(0, 5.75, r'H+CCN$\rightarrow$C+HCN', fontsize=12, color='white')
		ax.text(1, 4.75, r'HCNH$^+$+e$^-$$\rightarrow$H+HCN', fontsize=12, color='white')
	if action == 'destruction' and mol == 'CN':
		ax.text(-3., 4.56, r'O+CN$\rightarrow$N+CO', fontsize=12, color='white')
		ax.text(2, 5., r'CN+h$\nu\rightarrow$C+N', fontsize=12, color='white')
		ax.text(-3.2, 5.75, r'CN+N$\rightarrow$C+N$_2$', fontsize=12, color='white')
	if action == 'destruction' and mol == 'HCN':
		ax.text(-2, 5., r'HCN+C$^+$$\rightarrow$H+CNC$^+$', fontsize=12, color='white')
		ax.text(2.5, 4.75, r'HCN+h$\nu\rightarrow$H+CN', fontsize=12, color='white')
		ax.text(-5., 3.8, r'HCN+H$^+$$\rightarrow$H+HNC$^+$', fontsize=12)
		ax.text(-4., 6.01, r'HCN$^+$+HCO$^+$$\rightarrow$CO+HCNH$^+$', fontsize=12)
		plt.arrow(-3.3,6.2,-0.5,-0.3, head_width=0.04)
		plt.arrow(-3.3,3.8,-0.3,0.3, head_width=0.04)
