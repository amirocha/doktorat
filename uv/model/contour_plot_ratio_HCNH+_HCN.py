...
def make_picture(X, Y, Z):
	...
	ax.set_title(r'Chemical model parameters - HCNH+/HCN ratio')
	...
	plt.savefig('contour_plot_HCNH+_HCN.eps')
	plt.close()
	...
	for dens in N:
		...
		for av in AV_tot:
			if av in AV:
				...
				time_position = 124  
				HCNHplus_abudance = data[287].split()[time_position]  
				HCNHplus_abudance = remove_D_from_elem(HCNHplus_abudance)
				HCN_abudance = data[52].split()[time_position] 
				HCN_abudance = remove_D_from_elem(HCN_abudance)
				abudance_ratio = float(HCNHplus_abudance)/float(HCN_abudance)
				Z_rows.append(abudance_ratio)
			elif av not in AV:
				av_new = AV_new[i]
				data_new = read_data('./plot'+dens+'-'+av_new+'.dat')
				time_position = 124  
				HCNHplus_abudance_new = data_new[287].split()[time_position]  
				HCNHplus_abudance_new = remove_D_from_elem(HCNHplus_abudance_new)
				HCN_abudance_new = data_new[52].split()[time_position] 
				HCN_abudance_new = remove_D_from_elem(HCN_abudance_new)
				abudance_ratio_new = float(HCNHplus_abudance_new)/float(HCN_abudance_new)
