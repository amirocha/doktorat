...
def make_picture(X, Y, Z):
	...
	fig, ax = plt.subplots()
	CS = ax.contour(X, Y, Z, levels = [0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000])
	ax.clabel(CS, inline=1, fontsize=10)
	observations = ax.contourf(X, Y, Z, levels = [1,10])
	ax.set_title(r'Chemical model parameters - HCS+/HCN ratio')
	...
	plt.savefig('contour_plot_HCS+_HCN.eps')
	plt.close()
	...
	for dens in N:
		...
		for av in AV_tot:
			if av in AV:
				...
				time_position = 124  
				HCS_abudance = data[255].split()[time_position]  
				HCS_abudance = remove_D_from_elem(HCS_abudance)
				HCN_abudance = data[52].split()[time_position] 
				HCN_abudance = remove_D_from_elem(HCN_abudance)
				abudance_ratio = float(HCS_abudance)/float(HCN_abudance)
				Z_rows.append(abudance_ratio)
			elif av not in AV:
				av_new = AV_new[i]
				data_new = read_data('./plot'+dens+'-'+av_new+'.dat')
				time_position = 124  
				HCS_abudance_new = data_new[255].split()[time_position]  
				HCS_abudance_new = remove_D_from_elem(HCS_abudance_new)
				HCN_abudance_new = data_new[52].split()[time_position] 
				HCN_abudance_new = remove_D_from_elem(HCN_abudance_new)
				abudance_ratio_new = float(HCS_abudance_new)/float(HCN_abudance_new)
