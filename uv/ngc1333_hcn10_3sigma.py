...
rms = 7.911E-03 # rms taken from CLASS
rms_3 = 3*rms

# read the spectrum
spec_df = pd.read_table('ngc1333_hcn10_ave_spec.txt', delim_whitespace=True, header=None)

### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ### 3 SIGMA ###
# left (x1) and right (x2) ranges in which we are looking for minima 
x1_ran_df = spec_df[(spec_df[0] > -30.0) & (spec_df[0] < -20.)]    #change ranges!! 
x2_ran_df = spec_df[(spec_df[0] > -5.) & (spec_df[0] < -1.)]
...
"""
v_hcn10, Tmb_hcn10 = loadtxt('ngc1333_hcn10_ave_spec.txt', usecols=(0, 1), unpack=True, skiprows=1)
