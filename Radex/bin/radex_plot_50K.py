...
def write_to_file(limits1, limits2):
	file_out = open('RADEX_N_ratio.txt', 'a')
...
def make_picture(ratio, fluxes, low_obs_log, high_obs_log, low_obs_log_smm, high_obs_log_smm, limits1, limits2):
	...
	fig.set_figheight(8)
	fig.set_figwidth(8)
	plt.plot(ratio, fluxes, 'r-')
	rect_outflow = rec((-2, low_obs_log), 4, high_obs_log-low_obs_log, color = 'blue', rasterized = True, alpha = 0.8)
	rect_smm = rec((-2, low_obs_log_smm), 4, high_obs_log_smm-low_obs_log_smm, color = 'green', rasterized = True, alpha = 0.8)
	ax.add_patch(rect_outflow)
	ax.add_patch(rect_smm)
	ax.axvline(limits1[0], linestyle = '--', color = 'black')
	ax.axvline(limits1[1], linestyle = '--', color = 'black')
	ax.axvline(limits2[0], linestyle = '--', color = 'navy')
	ax.axvline(limits2[1], linestyle = '--', color = 'navy')
	plt.ylabel(r'$\log(\frac{I_\mathrm{CN}}{I_\mathrm{HCN}}$)', size=20)
	plt.xlabel(r'$\log(\frac{N_\mathrm{CN}}{N_\mathrm{HCN}}$)', size=20)
...
def main():
	...
	for col_dens in ['1e6', '1e7', '1e8', '1e9', '1e10']:
		...
		#flux_cs = float(read_data('./output/cs'+H2_dens+TEMP+col_dens+'.out'))
		#fluxes_cs.append(flux_cs)
	
	fluxes = [flux/flux_hcn for flux in fluxes_cn]
